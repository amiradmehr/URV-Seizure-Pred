"""
Validate the processed CHB-MIT dataset.
...
def load_processed_manifest() -> pd.DataFrame:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(
            ...
            "Run scripts/chb_mit/build_dataset.py first."
        )
...
def validate_patient_split_isolation() -> None:
    """Verify each observed patient belongs to its configured split only."""
    ...
    if missing_subjects:
        raise ValueError(
            "Configured subjects are absent from the decision manifest: "
            f"{missing_subjects}"
        )
...
def main() -> None:
    ...
    print_header("PROCESSED CHB-MIT VALIDATION")
    ...
    validate_patient_split_isolation()
    print("Patient split isolation: PASS")
