"""
Build the filtered, labeled, patient-level split CHB-MIT dataset.

Run from the repository root:

    python scripts/chb_mit/build_dataset.py
"""
...
from seizure_prediction.chb_mit.config import CONFIG  # noqa: E402
from seizure_prediction.chb_mit.preprocessing import (  # noqa: E402
...
def main() -> None:
    ...
    print_header("CHB-MIT PREPROCESSING PIPELINE")
    ...
    for recording_number, recording in enumerate(
        ...
        montage_status = infer_montage_status(raw)
        ...
        print(
            f"    Channels: {len(processed_raw.ch_names)} canonical derivations"
        )
    ...
    if unknown_positive_count > 0:
        print(
            ...
            "Do not guess these labels; CHB-MIT ships seizure onset "
            "and stop times only, so a scope column would have to come "
            "from a trusted external annotation source."
        )
    ...
    print(
        "\nNext command:\n"
        "    python scripts/chb_mit/validate_dataset.py"
    )
